...
# def init_whisperx(model_path="weights/speech2text", device="cuda", compute_type="float16"):
#     model = whisperx.load_model(model_path, device, compute_type=compute_type)
#     return model

# def transcribe_audio_with_whisperx(model_asr, audio_path: str, language: str = "vi") -> Optional[str]:
#     """Chuyển đổi audio thành văn bản bằng Faster Whisper"""
#     try:
#         audio = whisperx.load_audio(audio_path)
#         segments = model_asr.transcribe(audio, batch_size=8, language=language)
...
async def transcribe_api_whisper(
    audio_data: bytes,
    filename: str = "audio.wav",
    model: str = "convert-model-880000",
    language: str = "vi",
    task: str = "transcribe",
    batch_size: int = 8,
    chunk_size: int = 15,
    ...
    response = requests.post(config.ASR_API, files=files, data=data)
